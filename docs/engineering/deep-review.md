...
- **The budget is per round, not per comment** — ≈2,500 characters for a re-review, ≈4,000 for the first — because two comments that each pass a per-comment cap still land twice the text on the author. Over budget means **compress** — never drop a finding, and never grant an exception: "cut, or lose a real finding" is a false choice, since a 3,400-character finding compresses to 400 without losing its claim, its `file:line` or its consequence. The round is measured before posting.
...
- On a delta re-review it re-posts when there are new commits or its verdict changed (a stale-verdict correction), and carries a lens result forward only when the head SHA is byte-identical — a moved SHA, even a merge, re-runs the runtime lenses. **"I now have better evidence" is not a third trigger** — proving a standing finding harder is the previous round restated, so it is a short reply in that finding's thread, never a fresh review. The check runs **before composing**, not at delivery, because once the text exists "don't post" reads as discarding work.
- A round is one main comment plus its runtime follow-up. A third post correcting the first ("I understated the scope") means it shipped before its severities were settled — those get fixed while the text is a draft, and a genuine late correction is one line in the existing thread.
